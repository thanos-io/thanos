...
- [#9025](https://github.com/thanos-io/thanos/pull/9025): Compact/Store: with `--block-discovery-strategy=recursive`, the deletion, no-compact and no-downsample marker filters only fetch marker files the recursive listing has seen instead of probing every block; this removes one GET per block and per marker type (almost always a 404) from every metadata sync.
