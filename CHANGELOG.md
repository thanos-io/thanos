...
- [#8638](https://github.com/thanos-io/thanos/pull/8638): Query: expose Prometheus query timing stats (eval, sort, prep, inner-eval, queue, total) on the `QueryStats` gRPC message. Stats collection is opt-in via the new `enable_stats` flag on `QueryRequest` / `QueryRangeRequest`; pre-existing clients that do not set the flag will no longer receive the `samples_total` / `peak_samples` fields they may have been getting before.
