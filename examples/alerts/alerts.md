...
- alert: ThanosCompactMultipleRunning
  annotations:
    description: No more than one Thanos Compact instance should be running at once.
      There are {{$value}} in {{$labels.namespace}}
  ...
  expr: sum by (namespace) (up{job=~"thanos-compact.*"}) > 1
  for: 5m
  labels:
    severity: warning
...
- alert: ThanosCompactHighCompactionFailures
  annotations:
    description: Thanos Compact {{$labels.namespace}}/{{$labels.job}} is failing to
      execute {{$value | humanize}}% of compactions.
  ...
  expr: |
    (
      sum by (namespsace, job) (rate(thanos_compact_group_compactions_failures_total{job=~"thanos-compact.*"}[5m]))
    /
      sum by (namespsace, job) (rate(thanos_compact_group_compactions_total{job=~"thanos-compact.*"}[5m]))
    * 100 > 5
    )
  for: 15m
  labels:
...
- alert: ThanosCompactBucketHighOperationFailures
  annotations:
    description: Thanos Compact {{$labels.namespace}}/{{$labels.job}} Bucket is failing
  ...
  expr: |
    (
      sum by (namespsace, job) (rate(thanos_objstore_bucket_operation_failures_total{job=~"thanos-compact.*"}[5m]))
    /
      sum by (namespsace, job) (rate(thanos_objstore_bucket_operations_total{job=~"thanos-compact.*"}[5m]))
    * 100 > 5
    )
  for: 15m
  labels:
...
- alert: ThanosCompactHasNotRun
  annotations:
    description: Thanos Compact {{$labels.namespace}}/{{$labels.job}} has not uploaded
  ...
  expr: (time() - max by (namespace, job) (max_over_time(thanos_objstore_bucket_last_successful_upload_time{job=~"thanos-compact.*"}[24h])))
    / 60 / 60 > 24
  labels:
    severity: warning
```
...
- alert: ThanosRuleQueueIsDroppingAlerts
  ...
  expr: |
    sum by (namespace, job) (rate(thanos_alert_queue_alerts_dropped_total{job=~"thanos-rule.*"}[5m])) > 0
  for: 5m
  labels:
    severity: critical
- alert: ThanosRuleSenderIsFailingAlerts
  ...
  expr: |
    sum by (namespace, job) (rate(thanos_alert_sender_alerts_dropped_total{job=~"thanos-rule.*"}[5m])) > 0
  for: 5m
  labels:
    severity: critical
- alert: ThanosRuleHighRuleEvaluationFailures
  ...
  expr: |
    (
      sum by (namespace, job) (rate(prometheus_rule_evaluation_failures_total{job=~"thanos-rule.*"}[5m]))
    /
      sum by (namespace, job) (rate(prometheus_rule_evaluations_total{job=~"thanos-rule.*"}[5m]))
    * 100 > 5
    )
  for: 5m
  labels:
...
- alert: ThanosRuleHighRuleEvaluationWarnings
  ...
  expr: |
    sum by (namespace, job) (rate(thanos_rule_evaluation_with_warnings_total{job=~"thanos-rule.*"}[5m])) > 0
  for: 15m
  labels:
    severity: info
- alert: ThanosRuleRuleEvaluationLatencyHigh
  ...
  expr: |
    (
      sum by (namespace, job, instance, rule_group) (prometheus_rule_group_last_duration_seconds{job=~"thanos-rule.*"})
    >
      sum by (namespace, job, instance, rule_group) (prometheus_rule_group_interval_seconds{job=~"thanos-rule.*"})
    )
  for: 5m
  labels:
    severity: warning
- alert: ThanosRuleGrpcErrorRate
  ...
  expr: |
    (
      sum by (namespace, job) (rate(grpc_server_handled_total{grpc_code=~"Unknown|ResourceExhausted|Internal|Unavailable|DataLoss|DeadlineExceeded", job=~"thanos-rule.*"}[5m]))
    /
      sum by (namespace, job) (rate(grpc_server_started_total{job=~"thanos-rule.*"}[5m]))
    * 100 > 5
    )
  for: 5m
  labels:
...
- alert: ThanosRuleQueryHighDNSFailures
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_rule_query_apis_dns_failures_total{job=~"thanos-rule.*"}[5m]))
    /
      sum by (namespace, job) (rate(thanos_rule_query_apis_dns_lookups_total{job=~"thanos-rule.*"}[5m]))
    * 100 > 1
    )
  for: 15m
  labels:
...
- alert: ThanosRuleAlertmanagerHighDNSFailures
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_rule_alertmanagers_dns_failures_total{job=~"thanos-rule.*"}[5m]))
    /
      sum by (namespace, job) (rate(thanos_rule_alertmanagers_dns_lookups_total{job=~"thanos-rule.*"}[5m]))
    * 100 > 1
    )
  for: 15m
  labels:
...
- alert: ThanosRuleNoEvaluationFor10Intervals
  ...
  expr: |
    time() -  max by (namespace, job, group) (prometheus_rule_group_last_evaluation_timestamp_seconds{job=~"thanos-rule.*"})
    >
    10 * max by (namespace, job, group) (prometheus_rule_group_interval_seconds{job=~"thanos-rule.*"})
  for: 5m
  labels:
    severity: info
- alert: ThanosNoRuleEvaluations
  ...
  expr: |
    sum by (namespace, job) (rate(prometheus_rule_evaluations_total{job=~"thanos-rule.*"}[2m])) <= 0
      and
    sum by (namespace, job) (thanos_rule_loaded_rules{job=~"thanos-rule.*"}) > 0
  for: 3m
  labels:
    severity: critical
```
...
- alert: ThanosStoreGrpcErrorRate
  annotations:
    description: Thanos Store {{$labels.namespace}}/{{$labels.job}} is failing to
  ...
  expr: |
    (
      sum by (namespace, job) (rate(grpc_server_handled_total{grpc_code=~"Unknown|ResourceExhausted|Internal|Unavailable|DataLoss|DeadlineExceeded", job=~"thanos-store.*"}[5m]))
    /
...
- alert: ThanosStoreSeriesGateLatencyHigh
  annotations:
    description: Thanos Store {{$labels.namespace}}/{{$labels.job}} has a 99th percentile
      latency of {{ $value }} seconds for store series gate requests.
  ...
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(thanos_bucket_store_series_gate_duration_seconds_bucket{job=~"thanos-store.*"}[5m]))) > 2
    and
      sum by (namespace, job) (rate(thanos_bucket_store_series_gate_duration_seconds_count{job=~"thanos-store.*"}[5m])) > 0
    )
  for: 10m
  labels:
    severity: warning
- alert: ThanosStoreBucketHighOperationFailures
  annotations:
    description: Thanos Store {{$labels.namespace}}/{{$labels.job}} Bucket is failing
      to execute {{$value | humanize}}% of operations.
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_objstore_bucket_operation_failures_total{job=~"thanos-store.*"}[5m]))
    /
...
- alert: ThanosStoreObjstoreOperationLatencyHigh
  annotations:
    description: Thanos Store {{$labels.namespace}}/{{$labels.job}} Bucket has a 99th
      percentile latency of {{ $value }} seconds for the bucket operations.
  ...
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(thanos_objstore_bucket_operation_duration_seconds_bucket{job=~"thanos-store.*"}[5m]))) > 2
    and
      sum by (namespace, job) (rate(thanos_objstore_bucket_operation_duration_seconds_count{job=~"thanos-store.*"}[5m])) > 0
    )
  for: 10m
  labels:
    severity: warning
...
- alert: ThanosSidecarUnhealthy
  ...
  expr: |
    time() - max by (namespace, job, instance) (thanos_sidecar_last_heartbeat_success_time_seconds{job=~"thanos-sidecar.*"}) >= 600
  labels:
    severity: critical
```
...
- alert: ThanosQueryHttpRequestQueryErrorRateHigh
  annotations:
    description: Thanos Query {{$labels.namespace}}/{{$labels.job}} is failing to
  ...
  expr: |
    (
      sum by (namespace, job) (rate(http_requests_total{code=~"5..", job=~"thanos-query.*", handler="query"}[5m]))
    /
      sum by (namespace, job) (rate(http_requests_total{job=~"thanos-query.*", handler="query"}[5m]))
    ) * 100 > 5
  for: 5m
  labels:
    severity: critical
- alert: ThanosQueryHttpRequestQueryRangeErrorRateHigh
  ...
  expr: |
    (
      sum by (namespace, job) (rate(http_requests_total{code=~"5..", job=~"thanos-query.*", handler="query_range"}[5m]))
    /
      sum by (namespace, job) (rate(http_requests_total{job=~"thanos-query.*", handler="query_range"}[5m]))
    ) * 100 > 5
  for: 5m
  labels:
    severity: critical
- alert: ThanosQueryGrpcServerErrorRate
  ...
  expr: |
    (
      sum by (namespace, job) (rate(grpc_server_handled_total{grpc_code=~"Unknown|ResourceExhausted|Internal|Unavailable|DataLoss|DeadlineExceeded", job=~"thanos-query.*"}[5m]))
    /
...
- alert: ThanosQueryHighDNSFailures
  annotations:
    description: Thanos Query {{$labels.namespace}}/{{$labels.job}} have {{$value
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_query_store_apis_dns_failures_total{job=~"thanos-query.*"}[5m]))
    /
...
- alert: ThanosQueryInstantLatencyHigh
  annotations:
    description: Thanos Query {{$labels.namespace}}/{{$labels.job}} has a 99th percentile
  ...
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(http_request_duration_seconds_bucket{job=~"thanos-query.*", handler="query"}[5m]))) > 40
    and
      sum by (namespace, job) (rate(http_request_duration_seconds_bucket{job=~"thanos-query.*", handler="query"}[5m])) > 0
    )
  for: 10m
  labels:
    severity: critical
- alert: ThanosQueryRangeLatencyHigh
  annotations:
    description: Thanos Query {{$labels.namespace}}/{{$labels.job}} has a 99th percentile
      latency of {{$value}} seconds for range queries.
    runbook_url: https://github.com/thanos-io/thanos/tree/main/mixin/runbook.md#alert-name-thanosqueryrangelatencyhigh
    summary: Thanos Query has high latency for queries.
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(http_request_duration_seconds_bucket{job=~"thanos-query.*", handler="query_range"}[5m]))) > 90
    and
      sum by (namespace, job) (rate(http_request_duration_seconds_count{job=~"thanos-query.*", handler="query_range"}[5m])) > 0
    )
  for: 10m
  labels:
    severity: critical
...
- alert: ThanosReceiveHttpRequestErrorRateHigh
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} is failing to
      handle {{$value | humanize}}% of requests.
  ...
  expr: |
    (
      sum by (namespace, job) (rate(http_requests_total{code=~"5..", job=~"thanos-receive.*", handler="receive"}[5m]))
    /
      sum by (namespace, job) (rate(http_requests_total{job=~"thanos-receive.*", handler="receive"}[5m]))
    ) * 100 > 5
  for: 5m
  labels:
    severity: critical
- alert: ThanosReceiveHttpRequestLatencyHigh
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} has a 99th percentile
      latency of {{ $value }} seconds for requests.
  ...
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(http_request_duration_seconds_bucket{job=~"thanos-receive.*", handler="receive"}[5m]))) > 10
    and
      sum by (namespace, job) (rate(http_request_duration_seconds_count{job=~"thanos-receive.*", handler="receive"}[5m])) > 0
    )
  for: 10m
  labels:
    severity: critical
- alert: ThanosReceiveHighReplicationFailures
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} is failing to
      replicate {{$value | humanize}}% of requests.
  ...
  expr: |
    ...
    (
      (
        sum by (namespace, job) (rate(thanos_receive_replications_total{result="error", job=~"thanos-receive.*"}[5m]))
      /
        sum by (namespace, job) (rate(thanos_receive_replications_total{job=~"thanos-receive.*"}[5m]))
      )
      >
      (
        max by (namespace, job) (floor((thanos_receive_replication_factor{job=~"thanos-receive.*"}+1) / 2))
      /
        max by (namespace, job) (thanos_receive_hashring_nodes{job=~"thanos-receive.*"})
      )
    ) * 100
  for: 5m
  labels:
...
- alert: ThanosReceiveHighForwardRequestFailures
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} is failing to
      forward {{$value | humanize}}% of requests.
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_receive_forward_requests_total{result="error", job=~"thanos-receive.*"}[5m]))
    /
      sum by (namespace, job) (rate(thanos_receive_forward_requests_total{job=~"thanos-receive.*"}[5m]))
    ) * 100 > 20
  for: 5m
  labels:
    severity: warning
- alert: ThanosReceiveHighHashringFileRefreshFailures
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} is failing to
      refresh hashring file, {{$value | humanize}} of attempts failed.
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_receive_hashrings_file_errors_total{job=~"thanos-receive.*"}[5m]))
    /
      sum by (namespace, job) (rate(thanos_receive_hashrings_file_refreshes_total{job=~"thanos-receive.*"}[5m]))
    > 0
    )
  for: 15m
  labels:
...
- alert: ThanosReceiveConfigReloadFailure
  annotations:
    description: Thanos Receive {{$labels.namespace}}/{{$labels.job}} has not been
  ...
  expr: avg by (namespace, job) (thanos_receive_config_last_reload_successful{job=~"thanos-receive.*"})
    != 1
  for: 5m
  labels:
    severity: warning
...
- alert: ThanosBucketReplicateErrorRate
  annotations:
    description: Thanos Replicate in {{$labels.namespace}} failing to run, {{$value
  ...
  expr: |
    (
      sum by (namespace, job) (rate(thanos_replicate_replication_runs_total{result="error", job=~"thanos-bucket-replicate.*"}[5m]))
    / on (namespace) group_left
      sum by (namespace, job) (rate(thanos_replicate_replication_runs_total{job=~"thanos-bucket-replicate.*"}[5m]))
    ) * 100 >= 10
  for: 5m
  labels:
    severity: critical
- alert: ThanosBucketReplicateRunLatency
  annotations:
    description: Thanos Replicate {{$labels.namespace}}/{{$labels.job}} has a 99th
      percentile latency of {{ $value }} seconds for the replicate operations.
  ...
  expr: |
    (
      histogram_quantile(0.99, sum by (namespace, job, le) (rate(thanos_replicate_replication_run_duration_seconds_bucket{job=~"thanos-bucket-replicate.*"}[5m]))) > 20
    and
      sum by (namespace, job) (rate(thanos_replicate_replication_run_duration_seconds_bucket{job=~"thanos-bucket-replicate.*"}[5m])) > 0
    )
  for: 5m
  labels:
    severity: critical
...
- alert: ThanosBucketReplicateIsDown
  annotations:
    description: ThanosBucketReplicate has disappeared from {{$labels.namespace}}.
...
- alert: ThanosReceiveIsDown
  annotations:
    description: ThanosReceive has disappeared from {{$labels.namespace}}. Prometheus
      target for the component cannot be discovered.
...
- alert: ThanosSidecarIsDown
  annotations:
    description: ThanosSidecar has disappeared from {{$labels.namespace}}. Prometheus
      target for the component cannot be discovered.
