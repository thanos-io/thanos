...
func (wc *webConfig) registerFlag(cmd *kingpin.CmdClause) *webConfig {
	cmd.Flag("web.external-prefix",
		"Static prefix for all HTML links and redirect URLs in the web UI interface. Actual endpoints are still served on / or the web.route-prefix. This allows thanos web UI to be served behind a reverse proxy that strips a URL sub-path.").
...
func (wr *webRoute) registerFlag(cmd *kingpin.CmdClause) *webRoute {
	cmd.Flag("web.route-prefix", "Prefix for API and UI endpoints. This allows thanos UI to be served on a sub-path. This option is analogous to --web.route-prefix of Promethus.").
		Default("").StringVar(&wr.prefix)
	return wr
}
