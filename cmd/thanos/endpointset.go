...
func setupEndpointSet(
	...
	legacyFileSDCache := cache.New()
	{
		...
		if fileSD != nil {
			...
			ctxLegacyFileSD, cancelLegacyFileSD := context.WithCancel(context.Background())
			g.Add(func() error {
				fileSD.Run(ctxLegacyFileSD, fileSDUpdates)
				return nil

			}, func(err error) {
				cancelLegacyFileSD()
			})
			ctxUpdateLegacyFileSDUpdate, cancelLegacyFileSDUpdate := context.WithCancel(context.Background())
			g.Add(func() error {
				for {
					select {
					...
					case <-ctxUpdateLegacyFileSDUpdate.Done():
						return nil
					}
				}
			}, func(err error) {
				cancelLegacyFileSDUpdate()
			})
		}
	}
