...
func (rc *receiveConfig) registerFlag(cmd extkingpin.FlagClause) {
	...
	rc.retryAfterBackoff = extkingpin.ModelDuration(cmd.Flag("receive-retry-after-backoff", "Backoff for retry-after header, set if active-series-limiting is enabled").Default("5s"))

	cmd.Flag("receive-retry-after-jitter", "Jitter for retry-after backoff").Default("0.5").Float64Var(&rc.retryAfterJitter)
}
