...
func registerStore(app *extkingpin.App) {
	...
	conf.cachingBucketConfig = *extflag.RegisterPathOrContent(extflag.HiddenCmdClause(cmd), "store.caching-bucket.config",
	...
	cmd.Flag("store.grpc.series-sample-limit",
		...
		Default("0").Uint64Var(&conf.maxSampleCount)
	...
	cmd.Flag("sync-block-duration", "Repeat interval for syncing the blocks between local and remote view.").
		Default("3m").DurationVar(&conf.syncInterval)
	...
	cmd.Flag("block-sync-concurrency", "Number of goroutines to use when constructing index-cache.json blocks from object storage.").
		Default("20").IntVar(&conf.blockSyncConcurrency)
	...
	cmd.Flag("debug.advertise-compatibility-label", "If true, Store Gateway in addition to other labels, will advertise special \"@thanos_compatibility_store_type=store\" label set. This makes store Gateway compatible with Querier before 0.8.0").
		Hidden().Default("true").BoolVar(&conf.advertiseCompatibilityLabel)

	conf.selectorRelabelConf = *extkingpin.RegisterSelectorRelabelFlags(cmd)
	...
	cmd.Flag("store.index-header-posting-offsets-in-mem-sampling", "Controls what is the ratio of postings offsets store will hold in memory. "+
		...
		Hidden().Default(fmt.Sprintf("%v", store.DefaultPostingOffsetInMemorySampling)).IntVar(&conf.postingOffsetsInMemSampling)

	conf.consistencyDelay = *extkingpin.ModelDuration(cmd.Flag("consistency-delay", "Minimum age of all blocks before they are being read. Set it to safe value (e.g 30m) if your object storage is eventually consistent. GCS and S3 are (roughly) strongly consistent.").
		Default("0s"))

	conf.ignoreDeletionMarksDelay = *extkingpin.ModelDuration(cmd.Flag("ignore-deletion-marks-delay", "Duration after which the blocks marked for deletion will be filtered out while fetching blocks. "+
	...
	cmd.Flag("store.enable-index-header-lazy-reader", "If true, Store Gateway will lazy memory map index-header only once the block is required by a query.").
		Default("false").BoolVar(&conf.lazyIndexReaderEnabled)
	...
	cmd.Setup(func(g *run.Group, logger log.Logger, reg *prometheus.Registry, tracer opentracing.Tracer, _ <-chan struct{}, debugLogging bool) error {
		if conf.filterConf.MinTime.PrometheusTimestamp() > conf.filterConf.MaxTime.PrometheusTimestamp() {
			return errors.Errorf("invalid argument: --min-time '%s' can't be greater than --max-time '%s'",
				conf.filterConf.MaxTime, conf.filterConf.MinTime)
		}
...
func runStore(
	...
	ignoreDeletionMarkFilter := block.NewIgnoreDeletionMarkFilter(logger, bkt, conf.ignoreDeletionMarksDelay, conf.blockMetaFetchConcurrency)
