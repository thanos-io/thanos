...
func (bqss *blockQuerierSeriesSet) Next() bool {
	...
	currLabels := labelpb.ProtobufLabelsToPromLabels(bqss.series[bqss.next].Labels)
	...
	for bqss.next < len(bqss.series) && labels.Compare(currLabels, labelpb.ProtobufLabelsToPromLabels(bqss.series[bqss.next].Labels)) == 0 {
