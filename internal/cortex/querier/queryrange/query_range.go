...
func AnalyzesMerge(analysis ...*Analysis) *Analysis {
	...
	for _, a := range analysis[1:] {
		...
		for i := 0; i < len(elements) && i < len(rootElements); i++ {
			if rootElements[i].ExecutionTime != nil && analysis[i].ExecutionTime != nil {
				rootElements[i].ExecutionTime = durationpb.New(rootElements[i].ExecutionTime.AsDuration() + analysis[i].ExecutionTime.AsDuration())
			}
		}
	}
...
func (s *SampleStream) UnmarshalJSON(data []byte) error {
	...
	s.Labels = cortexpb.FromMetricsToLabelPairs(sampleStream.Metric)
...
func (s *SampleStream) MarshalJSON() ([]byte, error) {
	var sampleStream model.SampleStream
	sampleStream.Metric = cortexpb.FromLabelPairsToMetric(s.Labels)
...
func (s *Sample) UnmarshalJSON(data []byte) error {
	...
	s.Labels = cortexpb.FromMetricsToLabelPairs(sample.Metric)
	s.SampleValue = float64(sample.Value)
	s.Timestamp = int64(sample.Timestamp)

	if sample.Histogram != nil {
		s.Histogram = fromModelSampleHistogram(sample.Histogram)
	} else {
		s.Histogram = nil
	}
...
func (s *Sample) MarshalJSON() ([]byte, error) {
	var sample model.Sample
	sample.Metric = cortexpb.FromLabelPairsToMetric(s.Labels)
...
func matrixMerge(resps []*PrometheusResponse) []*SampleStream {
	...
	for _, resp := range resps {
		for _, stream := range resp.Data.Result {
			metric := cortexpb.FromLabelPairsToLabels(stream.Labels).String()
