...
var (
	...
	parsedResponse = &PrometheusResponse{
		...
		Data: &PrometheusData{
			...
			Result: []*SampleStream{
				{
					Labels: cortexpb.FromLabelAdaptersToLabelPairs([]cortexpb.LabelAdapter{
						{Name: "foo", Value: "bar"},
					}),
...
func TestStatsCacheQuerySamples(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expectedResponse, r, protocmp.Transform()); diff != "" {
...
func TestPartition(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Nil(t, err)
			if diff := cmp.Diff(tc.expectedRequests, reqs, protocmp.Transform()); diff != "" {
				t.Errorf("requests mismatch (-want +got):\n%s", diff)
			}
			if diff := cmp.Diff(tc.expectedCachedResponse, resps, protocmp.Transform()); diff != "" {
...
func TestHandleHit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expectedUpdatedCachedEntry, updatedExtents, protocmp.Transform()); diff != "" {
				t.Errorf("updated cache entry mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
...
func TestResultsCache(t *testing.T) {
	...
	// Create a deep copy to avoid modifying the global parsedResponse
	// Deep copy the Result slice and its contents
	resultCopy := make([]*SampleStream, len(parsedResponse.Data.Result))
	for i, ss := range parsedResponse.Data.Result {
		samplesCopy := make([]*cortexpb.Sample, len(ss.Samples))
		copy(samplesCopy, ss.Samples)
		resultCopy[i] = &SampleStream{
			Labels:     ss.Labels,
			Samples:    samplesCopy,
	...
	useResp := &PrometheusResponse{
		...
		Data: &PrometheusData{
			ResultType: parsedResponse.Data.ResultType,
			Result:     resultCopy,
			Stats:      parsedResponse.Data.Stats,
			Analysis:   &Analysis{},
		},
	...
	if diff := cmp.Diff(useResp, resp, protocmp.Transform()); diff != "" {
		t.Errorf("response mismatch (-want +got):\n%s", diff)
	}
...
func TestResultsCacheRecent(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, calls)
	if diff := cmp.Diff(parsedResponse, resp, protocmp.Transform()); diff != "" {
		t.Errorf("response mismatch (-want +got):\n%s", diff)
	}
...
func TestNativeHistograms(t *testing.T) {
	...
	// Create a deep copy to avoid modifying the global parsedHistogramResponse
	// Deep copy the Result slice and its contents
	histResultCopy := make([]*SampleStream, len(parsedHistogramResponse.Data.Result))
	for i, ss := range parsedHistogramResponse.Data.Result {
		histCopy := make([]*SampleHistogramPair, len(ss.Histograms))
		copy(histCopy, ss.Histograms)
		histResultCopy[i] = &SampleStream{
			Labels:     ss.Labels,
			Samples:    ss.Samples,
			Histograms: histCopy,
		}
	...
	useResp := &PrometheusResponse{
		Status: parsedHistogramResponse.Status,
		Data: &PrometheusData{
			ResultType: parsedHistogramResponse.Data.ResultType,
			Result:     histResultCopy,
			Stats:      parsedHistogramResponse.Data.Stats,
			Analysis:   &Analysis{},
		},
		Warnings: parsedHistogramResponse.Warnings,
	}
