        ...
        rules: [
          {
            ...
            expr: |||
              sum by (%(aggregators)s) (rate(thanos_alert_queue_alerts_dropped_total{%(selector)s}[5m])) > 0
            ||| % thanos.rule,
            'for': '5m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            ...
            expr: |||
              sum by (%(aggregators)s) (rate(thanos_alert_sender_alerts_dropped_total{%(selector)s}[5m])) > 0
            ||| % thanos.rule,
            'for': '5m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(prometheus_rule_evaluation_failures_total{%(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(prometheus_rule_evaluations_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: |||
              sum by (%(aggregators)s) (rate(thanos_rule_evaluation_with_warnings_total{%(selector)s}[5m])) > 0
            ||| % thanos.rule,
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s, instance, rule_group) (prometheus_rule_group_last_duration_seconds{%(selector)s})
              >
                sum by (%(aggregators)s, instance, rule_group) (prometheus_rule_group_interval_seconds{%(selector)s})
              )
            ||| % thanos.rule,
            'for': '5m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(grpc_server_handled_total{grpc_code=~"Unknown|ResourceExhausted|Internal|Unavailable|DataLoss|DeadlineExceeded", %(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(grpc_server_started_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: 'avg by (%(aggregators)s) (thanos_rule_config_last_reload_successful{%(selector)s}) != 1' % thanos.rule,
            'for': '5m',
            labels: {
              severity: 'info',
            },
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(thanos_rule_query_apis_dns_failures_total{%(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(thanos_rule_query_apis_dns_lookups_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(thanos_rule_alertmanagers_dns_failures_total{%(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(thanos_rule_alertmanagers_dns_lookups_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: |||
              time() -  max by (%(aggregators)s, group) (prometheus_rule_group_last_evaluation_timestamp_seconds{%(selector)s})
              >
              10 * max by (%(aggregators)s, group) (prometheus_rule_group_interval_seconds{%(selector)s})
            ||| % thanos.rule,
            'for': '5m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              sum by (%(aggregators)s) (rate(prometheus_rule_evaluations_total{%(selector)s}[2m])) <= 0
                and
              sum by (%(aggregators)s) (thanos_rule_loaded_rules{%(selector)s}) > 0
            ||| % thanos.rule,
            'for': '3m',
            labels: {
              severity: 'critical',
