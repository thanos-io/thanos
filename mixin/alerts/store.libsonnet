      ...
      local location = if std.length(std.objectFields(thanos.hierarcies)) > 0 then ' in ' + std.join('/', ['{{labels.%s}}' % level for level in std.objectFields(thanos.hierarcies)]) else ' ';
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              (
                histogram_quantile(0.99, sum by (%(aggregators)s, le) (rate(thanos_bucket_store_series_gate_duration_seconds_bucket{%(selector)s}[5m]))) > %(seriesGateErrorThreshold)s
              and
                sum by (%(aggregators)s) (rate(thanos_bucket_store_series_gate_duration_seconds_count{%(selector)s}[5m])) > 0
              )
            ||| % thanos.store,
            'for': '10m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(thanos_objstore_bucket_operation_failures_total{%(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(thanos_objstore_bucket_operations_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: |||
              (
                histogram_quantile(0.99, sum by (%(aggregators)s, le) (rate(thanos_objstore_bucket_operation_duration_seconds_bucket{%(selector)s}[5m]))) > %(bucketOpsP99LatencyThreshold)s
              and
                sum by (%(aggregators)s) (rate(thanos_objstore_bucket_operation_duration_seconds_count{%(selector)s}[5m])) > 0
              )
            ||| % thanos.store,
            'for': '10m',
            labels: {
