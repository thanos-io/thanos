        ...
        rules: [
          {
            ...
            expr: 'sum by (%(aggregators)s) (up{%(selector)s}) > 1' % thanos.compact,
            'for': '5m',
            labels: {
              severity: 'warning',
            },
          ...
          },
          {
            ...
            expr: |||
              (
                sum by (%(aggregators)s) (rate(thanos_compact_group_compactions_failures_total{%(selector)s}[5m]))
              /
                sum by (%(aggregators)s) (rate(thanos_compact_group_compactions_total{%(selector)s}[5m]))
          ...
          },
          {
            ...
            expr: '(time() - max by (%(aggregators)s) (max_over_time(thanos_objstore_bucket_last_successful_upload_time{%(selector)s}[24h]))) / 60 / 60 > 24' % thanos.compact,
            labels: {
              severity: 'warning',
            },
          },
