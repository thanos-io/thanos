  ...
  grafanaDashboards+:: {
    [if thanos.rule != null then 'rule.json']:
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.queryPanel(
            'sum by (%s) (rate(prometheus_rule_evaluations_total{%s}[$interval]))' % [utils.joinLabels([thanos.rule.dashboard.aggregator, 'strategy']), thanos.rule.dashboard.selector],
            '{{ strategy }}',
          )
        )
        .addPanel(
          ...
          g.queryPanel(
            'sum by (%s) (increase(prometheus_rule_group_iterations_missed_total{%s}[$interval]))' % [utils.joinLabels([thanos.rule.dashboard.aggregator, 'strategy']), thanos.rule.dashboard.selector],
            '{{ strategy }}',
          )
        )
        .addPanel(
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.grpcRequestsPanel('grpc_server_handled_total', grpcUnarySelector, thanos.rule.dashboard.aggregator)
        )
        .addPanel(
          g.panel('Errors', 'Shows ratio of errors compared to the total number of handled requests.') +
          g.grpcErrorsPanel('grpc_server_handled_total', grpcUnarySelector, thanos.rule.dashboard.aggregator)
        )
        .addPanel(
          g.panel('Duration', 'Shows how long has it taken to handle requests, in quantiles.') +
          g.latencyPanel('grpc_server_handling_seconds', grpcUnarySelector, thanos.rule.dashboard.aggregator)
        )
      )
      .addRow(
        g.row('gRPC (Stream)')
        .addPanel(
          ...
          g.grpcRequestsPanel('grpc_server_handled_total', grpcServerStreamSelector, thanos.rule.dashboard.aggregator)
        )
        .addPanel(
          g.panel('Errors', 'Shows ratio of errors compared to the total number of handled requests.') +
          g.grpcErrorsPanel('grpc_server_handled_total', grpcServerStreamSelector, thanos.rule.dashboard.aggregator)
        )
        .addPanel(
          g.panel('Duration', 'Shows how long has it taken to handle requests, in quantiles') +
          g.latencyPanel('grpc_server_handling_seconds', grpcServerStreamSelector, thanos.rule.dashboard.aggregator)
        )
      )
      .addRow(
    ...
    __overviewRows__+:: [
      ...
      .addPanel(
        ...
        g.queryPanel(
          'sum by (%s) (rate(thanos_alert_sender_alerts_sent_total{%s}[$interval]))' % [utils.joinLabels([thanos.dashboard.overview.aggregator, 'alertmanager']), thanos.dashboard.overview.selector],
          '{{alertmanager}}'
        ) +
