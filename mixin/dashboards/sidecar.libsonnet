    ...
    [if thanos.sidecar != null then 'sidecar.json']:
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.latencyPanel('grpc_server_handling_seconds_bucket', '%s, grpc_type="unary"' % thanos.sidecar.dashboard.selector, thanos.sidecar.dashboard.aggregator)
        )
      )
      .addRow(
        g.row('gRPC (Stream)')
        ...
        .addPanel(
          g.panel('Duration', 'Shows how long has it taken to handle requests from queriers, in quantiles.') +
          g.latencyPanel('grpc_server_handling_seconds_bucket', '%s, grpc_type="server_stream"' % thanos.sidecar.dashboard.selector, thanos.sidecar.dashboard.aggregator)
        )
      )
      .addRow(
        g.row('Last Updated')
