...
func TestQueryNativeHistograms(t *testing.T) {
	e, err := e2e.NewDockerEnvironment("native-hist")
	testutil.Ok(t, err)
	t.Cleanup(e2ethanos.CleanScenario(t, e))
	...
	prom1, sidecar1 := e2ethanos.NewPrometheusWithSidecar(e, "alone", e2ethanos.DefaultPromConfig("prom-alone", 0, "", "", e2ethanos.LocalPrometheusTarget), "", "quay.io/prometheus/prometheus:v2.40.5", "", "native-histograms", "remote-write-receiver")
	testutil.Ok(t, e2e.StartAndWaitReady(prom1, sidecar1))

	// Querier. Both fileSD and directly by flags.
	querier := e2ethanos.NewQuerierBuilder(e, "1", sidecar1.InternalEndpoint("grpc"), receiver.InternalEndpoint("grpc")).Init()
	testutil.Ok(t, e2e.StartAndWaitReady(querier))

	remoteWriteURL, err := url.Parse("http://" + prom1.Endpoint("http") + "/api/v1/write")
	testutil.Ok(t, err)
	...
	testHistogram := remote.HistogramToHistogramProto(time.Now().UnixMilli(), &histogram.Histogram{
		Count:         5,
		ZeroCount:     2,
		Sum:           18.4,
