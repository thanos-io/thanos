...
func TestRulesAPI_Fanout(t *testing.T) {
	...
	prom1, sidecar1, err := e2ethanos.NewPrometheusWithSidecar(
		e,
		"prom1",
		e2ethanos.DefaultPromConfig("ha", 0, "", filepath.Join(e2ethanos.ContainerSharedDir, promRulesSubDir, "*.yaml"), true),
		"",
	...
	prom2, sidecar2, err := e2ethanos.NewPrometheusWithSidecar(
		e,
		"prom2",
		e2ethanos.DefaultPromConfig("ha", 1, "", filepath.Join(e2ethanos.ContainerSharedDir, promRulesSubDir, "*.yaml"), true),
		"",
		e2ethanos.DefaultPrometheusImage(), "",
	)
	testutil.Ok(t, err)
