...
type EndpointsConfig struct {
	...
	FileSDConfigs []exthttp.FileSDConfig `yaml:"file_sd_configs"`
}

// NewDiscoverer returns a new exthttp.Discoverer.
func NewDiscoverer(logger log.Logger, cfg EndpointsConfig, provider exthttp.AddressProvider) (*exthttp.Discoverer, error) {
	return exthttp.NewDiscoverer(logger, exthttp.EndpointsConfig{
...
func ClientGRPCOpts(logger log.Logger, tracer opentracing.Tracer, metrics *grpc_prometheus.ClientMetrics, config exthttp.ClientConfig) ([]grpc.DialOption, error) {
	...
	if (config.BasicAuth != exthttp.BasicAuth{} || config.BearerToken != "" || config.BearerTokenFile != "" || config.ProxyURL != "") {
		return nil, errors.New("basic auth, bearer token and proxy URL options are currently not implemented")

	}

	if (config.TLSConfig == exthttp.TLSConfig{}) {
