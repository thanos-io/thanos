	...
	default:
		{
			...
			chunk := 0
			uploadEveryPart := func(everypartsize int64, cnk int) (api.UploadInfoType, error) {
				...
				if err != nil {
					if err := b.client.AbortMultipartUpload(b.name, name, init.UploadId); err != nil {
						return api.UploadInfoType{cnk, etag}, errors.Wrap(err, "failed to abort multi-part upload")
					}

					return api.UploadInfoType{cnk, etag}, errors.Wrap(err, "failed to upload multi-part chunk")
				}
				return api.UploadInfoType{cnk, etag}, nil
			}
...
//		var marker string
//		for {
//			result, err := b.client.ListObjects(b.name, &api.ListObjectsArgs{
//				objstore.DirDelim,
//				marker,
//				1000,
//				objectPrefix,
