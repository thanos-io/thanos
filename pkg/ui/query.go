...
func NewQueryUI(logger log.Logger, reg prometheus.Registerer, storeSet *query.StoreSet, flagsMap map[string]string) *Query {
	...
	return &Query{
		BaseUI:   NewBaseUI(logger, "query_menu.html", queryTmplFuncs(), flagsMap, component.Query),
		storeSet: storeSet,
		cwd:      cwd,
		birth:    time.Now(),
		reg:      reg,
...
func (q *Query) Register(r *route.Router, ins extpromhttp.InstrumentationMiddleware) {
	...
	r.Get("/new", func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, path.Join(GetWebPrefix(q.logger, q.flagsMap, r), "new")+"/", http.StatusFound)
	})
