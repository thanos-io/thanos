...
type Issue func(ctx context.Context, logger log.Logger, bkt objstore.Bucket, repair bool) error
...
func NewWithRepair(logger log.Logger, bkt objstore.Bucket, issues []Issue) *Verifier {
	return &Verifier{
		logger: logger,
		bkt:    bkt,
		issues: issues,
