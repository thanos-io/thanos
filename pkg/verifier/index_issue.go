	...
	for id, meta := range metas {
		...
		defer func() {
			if err := os.RemoveAll(tmpdir); err != nil {
				_ = level.Warn(ctx.Logger).Log("msg", "failed to delete dir", "tmpdir", tmpdir, "err", err)
			}
		}()
		...
		_ = level.Debug(ctx.Logger).Log("stats", fmt.Sprintf("%+v", stats), "id", id)
		if err = stats.AnyErr(); err == nil {
			return nil
		}

		_ = level.Warn(ctx.Logger).Log("msg", "detected issue", "id", id, "err", err)
		...
		if stats.OutOfOrderChunks > stats.DuplicatedChunks {
			_ = level.Warn(ctx.Logger).Log("msg", "detected overlaps are not entirely by duplicated chunks. We are able to repair only duplicates", "id", id)
		}

		if stats.OutsideChunks > (stats.CompleteOutsideChunks + stats.Issue347OutsideChunks) {
			_ = level.Warn(ctx.Logger).Log("msg", "detected outsiders are not all 'complete' outsiders or outsiders from https://github.com/prometheus/tsdb/issues/347. We can safely delete only these outsiders", "id", id)
		}
		...
		_ = level.Info(ctx.Logger).Log("msg", "downloading block for repair", "id", id)
		...
		_ = level.Info(ctx.Logger).Log("msg", "downloaded block to be repaired", "id", id, "issue")

		_ = level.Info(ctx.Logger).Log("msg", "repairing block", "id", id, "issue")
		...
		_ = level.Info(ctx.Logger).Log("msg", "verifying repaired block", "id", id, "newID", resid)
		...
		_ = level.Info(ctx.Logger).Log("msg", "uploading repaired block", "newID", resid)
		...
		_ = level.Info(ctx.Logger).Log("msg", "safe deleting broken block", "id", id, "issue")
		...
		_ = level.Info(ctx.Logger).Log("msg", "all good, continuing", "id", id)
		return nil
	}

	_ = level.Info(ctx.Logger).Log("msg", "verified issue", "with-repair", repair)
	return nil
}
