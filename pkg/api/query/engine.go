...
func NewQueryFactory(
	...
	opts := func(registry prometheus.Registerer) engine.Opts {
		res := engine.Opts{
		...
		if activeQueryTracker != nil {
			res.ActiveQueryTracker = activeQueryTracker
		}
		return res
	}

	promEngine := promql.NewEngine(opts(extprom.WrapRegistererWith(
		map[string]string{
			"mode":   string(PromqlQueryModeLocal),
			"engine": string(PromqlEnginePrometheus)}, reg)).EngineOpts)

	thanosLocal := engine.New(opts(extprom.WrapRegistererWith(
		map[string]string{
			"mode":   string(PromqlQueryModeLocal),
			"engine": string(PromqlEngineThanos)}, reg)))
	thanosDistributed := engine.NewDistributedEngine(opts(extprom.WrapRegistererWith(
		map[string]string{
