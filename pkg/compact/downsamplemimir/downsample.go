...
func Downsample(
	...
	for postings.Next() {
		...
		if origMeta.Thanos.Downsample.Resolution == 0 {
			...
			if err := streamedBlockWriter.WriteSeries(lset.Labels(), DownsampleRaw(all, resolution)); err != nil {
			...
			if err := streamedBlockWriter.WriteSeries(lset.Labels(), downsampledChunks); err != nil {
