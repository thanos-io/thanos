...
func (p *tsdbBasedPlanner) Plan(_ context.Context, metasByMinTime []*metadata.Meta) ([]*metadata.Meta, error) {
	...
	for _, meta := range selectMetas(p.ranges, metasByMinTime) {
		res = append(res, meta)
	}
	if len(res) > 0 {
		return res, nil
...
type indexSizeAwarePlanner struct {
	tsdbBasedPlanner  *tsdbBasedPlanner
	bkt               objstore.BucketReader
	maxIndexSizeBytes int64
}

var _ Planner = &indexSizeAwarePlanner{}

func NewIndexSizeAwarePlanner(tsdbBasedPlanner *tsdbBasedPlanner, bkt objstore.BucketReader, maxIndexSizeBytes int64) *indexSizeAwarePlanner {
	return &indexSizeAwarePlanner{tsdbBasedPlanner: tsdbBasedPlanner, bkt: bkt, maxIndexSizeBytes: maxIndexSizeBytes}
}
func (s *indexSizeAwarePlanner) Plan(ctx context.Context, metasByMinTime []*metadata.Meta) ([]*metadata.Meta, error) {
	plan, err := s.tsdbBasedPlanner.Plan(ctx, metasByMinTime)
	if err != nil {
		return nil, err
	}
	...
	var totalSize int64
	for _, p := range plan {
		attr, err := s.bkt.Attributes(ctx, filepath.Join(p.ULID.String(), "index"))
		if err != nil {
			return nil, errors.Wrapf(err, "attr of %v", filepath.Join(p.ULID.String(), "index"))
		}
		totalSize += attr.Size
