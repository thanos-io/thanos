...
func TestGatherNoCompactionMarkFilter_WithListedMarkers(t *testing.T) {
	...
	listed.Set(map[ulid.ULID]map[string]struct{}{id2: {metadata.NoCompactMarkFilename: {}}})
	f := NewGatherNoCompactionMarkFilter(log.NewNopLogger(), objstore.WithNoopInstr(bkt), 4).WithListedMarkers(listed)
	...
	f = NewGatherNoCompactionMarkFilter(log.NewNopLogger(), objstore.WithNoopInstr(bkt), 4).WithListedMarkers(block.NewListedMarkers())
	testutil.Ok(t, f.Filter(ctx, metas(), synced, nil))
	testutil.Equals(t, 2, len(f.NoCompactMarkedBlocks()))

	// Without listed markers at all (default) every block is probed.
	f = NewGatherNoCompactionMarkFilter(log.NewNopLogger(), objstore.WithNoopInstr(bkt), 4)
	testutil.Ok(t, f.Filter(ctx, metas(), synced, nil))
	testutil.Equals(t, 2, len(f.NoCompactMarkedBlocks()))
}
