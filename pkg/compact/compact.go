...
func (g *DefaultGrouper) Groups(blocks map[ulid.ULID]*metadata.Meta) (res []*Group, err error) {
	...
	for _, m := range blocks {
		...
		if !ok {
			...
			group, err = NewGroup(
				log.With(g.logger, "group", fmt.Sprintf("%s@%v", resolutionLabel, lbls.String()), "groupKey", groupKey, "resolution", metadata.ResolutionToString(resolutionVal), "compact level", m.Compaction.Level),
				g.bkt,
				groupKey,
				lbls,
