...
func (c *Syncer) groupKey(meta *block.Meta) string {
	return fmt.Sprintf("%x-%d", labels.FromMap(meta.Thanos.Labels).Hash(), meta.Thanos.DownsamplingWindow)
}
...
func (c *Syncer) add(bdir string) error {
	...
	if !ok {
		g, err = newGroup(
			...
			meta.Thanos.Labels,
			meta.Thanos.DownsamplingWindow,
		)
		if err != nil {
...
func (c *Syncer) garbageCollect(ctx context.Context) error {
	...
	for id, meta := range all {
		...
		for _, sid := range meta.Compaction.Sources {
			...
			if meta.Thanos.DownsamplingWindow == pmeta.Thanos.DownsamplingWindow &&
				(level > plevel || (level == plevel && id.Compare(pid) > 0)) {
				parents[sid] = id
			}
		}
	}
...
// Downsample older blocks. This method is only relevant to backfill downsample blocks. They
// are usually generated while compacting blocks in groups.
func (c *Syncer) Downsample(ctx context.Context) {
...
func newGroupMetrics(reg prometheus.Registerer, labels map[string]string, getBlocksFunc func() (ids []ulid.ULID, err error)) *groupMetrics {
	...
	// We cannot just append labels as metric labels, because it will conflict with the things generated
	// by given Thanos source represent by this group.
...
func (cg *Group) Add(meta *block.Meta) error {
	...
	if meta.Thanos.DownsamplingWindow != cg.downWin {
		return errors.New("downsampling window does not match")
	}
...
func (cg *Group) Compact(ctx context.Context, comp tsdb.Compactor) (ulid.ULID, error) {
	...
	// If the block we just created is of the initial range for downsampling, create
	// the downsampled block right away and upload that too.
	// Downsample(ctx, filepath.Join(cg.dir, id.String()), cg.dir)
