...
func (p *tsdbPlannerAdapter) Plan(_ context.Context, metasByMinTime []*metadata.Meta) ([]*metadata.Meta, error) {
	...
	for _, meta := range metasByMinTime {
		...
		if err := metadata.Write(log.NewNopLogger(), bdir, meta); err != nil {
...
func TestPlanners_Plan_Compatibility(t *testing.T) {
	...
	// Mock small sizes of index to not trigger any special handling for this compatibility test.
	bkt := objstore.NewInMemBucket()
	for i := uint64(1); i < 10; i++ {
		testutil.Ok(t, bkt.Upload(nil, filepath.Join(ulid.MustNew(i, nil).String(), "index"), bytes.NewReader([]byte("index"))))
	}
	sizePlanner := &indexSizeAwarePlanner{
		tsdbBasedPlanner:  tsdbBasedPlanner,
		bkt:               bkt,
		maxIndexSizeBytes: 100,
	}
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			t.Run("indexSizeAwarePlanner", func(t *testing.T) {
				metasByMinTime := make([]*metadata.Meta, len(c.metas))
				for i := range metasByMinTime {
					metasByMinTime[i] = c.metas[i]
				}
				...
				plan, err := sizePlanner.Plan(context.Background(), metasByMinTime)
				testutil.Ok(t, err)
				testutil.Equals(t, c.expected, plan)
			})
		})
...
func TestRangeWithFailedCompactionWontGetSelected(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			t.Run("indexSizeAwarePlanner", func(t *testing.T) {
				plan, err := sizePlanner.Plan(context.Background(), c.metas)
				testutil.Ok(t, err)
				testutil.Equals(t, []*metadata.Meta(nil), plan)
			})
		})
