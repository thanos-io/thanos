...
type tenant struct {
	...
	lastSuccessfulHeadFlush time.Time
}
...
func (t *tenant) shouldBeMarkedInactive() bool {
	...
	if time.Since(t.lastSuccessfulHeadFlush) > 6*time.Hour {
		if !t.lastSuccessfulHeadFlush.IsZero() {
			level.Error(t.logger).Log("msg", "compaction is stuck so not marking tenant as inactive", "now", time.Now(), "last_compaction", t.lastSuccessfulHeadFlush, "diff", time.Since(t.lastSuccessfulHeadFlush))
		}
		return false
	}
...
func (t *tenant) startPeriodicHeadFlush() {
	...
	doIter := func() error {
		...
		flushHeadThreshold := int64(1.5 * float64(t.maxBlockDuration))
		if sinceLastAppendMillis > flushHeadThreshold {
			if err := t.flushHead(db); err != nil {
				return fmt.Errorf("flush head: %w", err)
			}
			t.lastSuccessfulHeadFlush = time.Now()
		}
	...
	go func() {
		level.Info(t.logger).Log("msg", "starting periodic head flush", "initial_delay", compactionDelay.String(), "interval", interval.String())
		select {
		...
		for {
			select {
			case <-ticker.C:
				level.Info(t.logger).Log("msg", "running periodic head flush")
				if err := doIter(); err != nil {
					level.Error(t.logger).Log("msg", "periodic flush failed", "err", err)
				}
