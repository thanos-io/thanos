...
func LoadConfig(confYAML []byte, endpointAddrs, strictEndpointAddrs []string, globalFileSDConfig *file.SDConfig, globalTLSConfig httpconfig.TLSConfig) ([]EndpointConfig, error) {
	...
	if len(endpointAddrs) > 0 || globalFileSDConfig != nil {
		...
		if globalFileSDConfig != nil {
			cfg.EndpointsConfig.FileSDConfigs = []httpconfig.FileSDConfig{
				{
