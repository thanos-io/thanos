...
func TestDedupRules(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "preserve recording rule order",
			rules: []*rulespb.Rule{
				rulespb.NewRecordingRule(&rulespb.RecordingRule{Name: "good_events:rate_2m"}),
				rulespb.NewRecordingRule(&rulespb.RecordingRule{Name: "total_events:rate_2m"}),
				rulespb.NewRecordingRule(&rulespb.RecordingRule{Name: "good_events:rate_1h"}),
			},
			want: []*rulespb.Rule{
				rulespb.NewRecordingRule(&rulespb.RecordingRule{Name: "good_events:rate_2m"}),
				rulespb.NewRecordingRule(&rulespb.RecordingRule{Name: "total_events:rate_2m"}),
