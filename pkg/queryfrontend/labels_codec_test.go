	...
	for i := 0; i < size; i++ {
		...
		seriesResp = append(
			...
			&ThanosSeriesResponse{
				Status: "success",
				Data:   []*labelpb.ZLabelSet{{Labels: []*labelpb.Label{{Name: fmt.Sprintf("foo-%d", i), Value: fmt.Sprintf("bar-%d", i)}}}},
			},
			&ThanosSeriesResponse{
				Status: "success",
				Data:   []*labelpb.ZLabelSet{{Labels: []*labelpb.Label{{Name: fmt.Sprintf("foo-%d", i+1), Value: fmt.Sprintf("bar-%d", i+1)}}}},
			},
		)
	}
