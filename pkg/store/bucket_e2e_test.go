...
func prepareStoreWithTestBlocks(t testing.TB, dir string, bkt objstore.Bucket, manyParts bool, chunksLimiterFactory ChunksLimiterFactory, seriesLimiterFactory SeriesLimiterFactory, bytesLimitersFactory BytesLimitersFactory, relabelConfig []*relabel.Config, filterConf *FilterConfig) *storeSuite {
...
func TestBucketStore_e2e(t *testing.T) {
	objtesting.ForeachStore(t, func(t *testing.T, bkt objstore.Bucket) {
		...
		s := prepareStoreWithTestBlocks(t, dir, bkt, false, NewChunksLimiterFactory(0), NewSeriesLimiterFactory(0), DefaultBytesLimitersFactory(0), emptyRelabelConfig, allowAllFilterConf)
...
func TestBucketStore_ManyParts_e2e(t *testing.T) {
	objtesting.ForeachStore(t, func(t *testing.T, bkt objstore.Bucket) {
		...
		s := prepareStoreWithTestBlocks(t, dir, bkt, true, NewChunksLimiterFactory(0), NewSeriesLimiterFactory(0), DefaultBytesLimitersFactory(0), emptyRelabelConfig, allowAllFilterConf)
...
func TestBucketStore_TimePartitioning_e2e(t *testing.T) {
	...
	s := prepareStoreWithTestBlocks(t, dir, bkt, false, NewChunksLimiterFactory(expectedChunks), NewSeriesLimiterFactory(0), DefaultBytesLimitersFactory(0), emptyRelabelConfig, &FilterConfig{
		MinTime: minTimeDuration,
...
func TestBucketStore_Series_ChunksLimiter_e2e(t *testing.T) {
	...
	for testName, testData := range cases {
		t.Run(testName, func(t *testing.T) {
			...
			s := prepareStoreWithTestBlocks(t, dir, bkt, false, NewChunksLimiterFactory(testData.maxChunksLimit), NewSeriesLimiterFactory(testData.maxSeriesLimit), DefaultBytesLimitersFactory(units.Base2Bytes(testData.maxBytesLimit)), emptyRelabelConfig, allowAllFilterConf)
			testutil.Ok(t, s.store.SyncBlocks(ctx))
...
func TestBucketStore_LabelNames_SeriesLimiter_e2e(t *testing.T) {
	...
	for testName, testData := range cases {
		t.Run(testName, func(t *testing.T) {
			...
			bkt := objstore.NewInMemBucket()
			dir := t.TempDir()
			s := prepareStoreWithTestBlocks(t, dir, bkt, false, NewChunksLimiterFactory(0), NewSeriesLimiterFactory(testData.maxSeriesLimit), DefaultBytesLimitersFactory(0), emptyRelabelConfig, allowAllFilterConf)
			testutil.Ok(t, s.store.SyncBlocks(ctx))
			req := &storepb.LabelNamesRequest{
				Matchers: []storepb.LabelMatcher{
					{Type: storepb.LabelMatcher_EQ, Name: "a", Value: "1"},
