...
type ProxyStore struct {
	logger         log.Logger
	stores         func() []Client
	rules          func() []storepb.RulesClient
...
func (s *ProxyStore) Rules(req *storepb.RulesRequest, srv storepb.Rules_RulesServer) error {
	var (
		g, gctx  = errgroup.WithContext(srv.Context())
		respChan = make(chan *storepb.RuleGroup, 10)
	...
	for _, g := range groups {
		if err := srv.Send(storepb.NewRuleGroupRulesResponse(g)); err != nil {
...
func dedupGroups(groups []*storepb.RuleGroup) []*storepb.RuleGroup {
...
func (stream *rulesStream) receive(ctx context.Context) error {
	...
	if err != nil {
		...
		if err := stream.server.Send(storepb.NewWarningRulesResponse(err)); err != nil {
			return err
		}
	}
	...
	for {
		...
		if w := rule.GetWarning(); w != "" {
			if err := stream.server.Send(storepb.NewWarningRulesResponse(errors.New(w))); err != nil {
