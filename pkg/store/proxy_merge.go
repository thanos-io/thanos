...
func NewProxyResponseLoserTree(seriesSets ...respSet) *proxyResponseLoserTree {
	...
	at := func(s respSet) seriesResponseWithStoreLabelset {
		return seriesResponseWithStoreLabelset{response: s.At(), labelset: s.Labelset()}
	}

	close := func(s respSet) { s.Close() }

	return &proxyResponseLoserTree{tree: losertree.New(seriesSets, maxVal, at, less, close)}
}
