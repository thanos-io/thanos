...
func (p *PrometheusStore) Series(r *storepb.SeriesRequest, seriesSrv storepb.Store_SeriesServer) error {
	...
	if r.SkipChunks {
		...
		for _, lbm := range labelMaps {
			...
			lset := labelpb.PromLabelsToLabels(b.Labels())
...
func (p *PrometheusStore) handleSampledPrometheusResponse(
	...
	for _, e := range resp.Results[0].Timeseries {
		...
		lset := rmLabels(labelpb.ExtendSortedLabels(labelpb.LabelsToPromLabels(e.Labels), extLset), extLsetToRemove)
		if len(e.Samples) == 0 {
		...
		if err := s.Send(storepb.NewSeriesResponse(&storepb.Series{
			Labels: labelpb.PromLabelsToLabels(lset),
...
func (p *PrometheusStore) handleStreamedPrometheusResponse(
	...
	for {
		...
		for _, series := range res.ChunkedSeries {
			...
			completeLabelset := rmLabels(labelpb.ExtendSortedLabels(labelpb.LabelsToPromLabels(series.Labels), extLset), extLsetToRemove)
			...
			r := storepb.NewSeriesResponse(&storepb.Series{
				Labels: labelpb.PromLabelsToLabels(completeLabelset),
				Chunks: thanosChks,
			})
...
func (p *PrometheusStore) chunkSamples(series *prompb.TimeSeries, maxSamplesPerChunk int, calculateChecksums bool) (chks []*storepb.AggrChunk, err error) {
	...
	for len(samples) > 0 {
		...
		chks = append(chks, &storepb.AggrChunk{
			MinTime: samples[0].GetTimestamp(),
			MaxTime: samples[chunkSize-1].GetTimestamp(),
...
func matchesExternalLabels(ms []*storepb.LabelMatcher, externalLabels labels.Labels, cache storecache.MatchersCache) (bool, []*labels.Matcher, error) {
	...
	tms, err = storecache.MatcherPointersToPromMatchersCached(cache, ms)
	if err != nil {
...
func (p *PrometheusStore) LabelSet() []*labelpb.LabelSet {
	extLabels := p.externalLabelsFn()

	var labelset []*labelpb.LabelSet
	ls := labelpb.PromLabelsToLabelSet(extLabels)
	if ls != nil {
		labelset = append(labelset, ls)
	}
