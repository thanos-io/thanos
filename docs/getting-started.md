...
Welcome to Thanos Documentation! 👋
...
In this quick guide, we will explain:
...
This documentation base is available in two versions. You are welcome to explore it on our [website](https://thanos.io/getting-started.md/) as well as on [GitHub directly](https://github.com/thanos-io/thanos/blob/master/docs/getting-started.md).

As presented on the left side menu (and in `docs` directory on Thanos repo), documentation is divided on a few sections:

### Thanos

General docs for Thanos overview and quick start.
### Components

The definitive list of all components Thanos binary gives. Note that the configurations is up to date with given
docs version as the configuration docs is autogenerated.

Go there if you want to know details about certain component (e.g Querier)

### Operating

Useful SOPs/Runbooks, tips and explanations from production operational side of Thanos.

### Contributing

List of resources useful if you want to help us improving this project! ❤️ This includes all kinds of contributions: docs,
code and tools.

### Proposals

The definitive list of current and past proposals that were made in Thanos project lifetime.

Proposals can have 5 Statuses (`.Params.Status`): `accepted`, `complete`, `rejected`, `in-review`, `draft`

Each proposal file should be in the format of `<data>-<name>.md`.
See details on proposal process [here](/contributing/#adding-new-features-components)
...
NOTE: Thanos will work in cloud native environments as well as more traditional ones. Some users run Thanos in Kubernetes while others on the bare metal.
