...
Thanos Store Gateway exports metrics about its own operation: block lifecycle, query path performance, caches, index-header handling, and request limits. The tables below list key metrics defined in the Store Gateway code paths.

**Notes**:

* Metrics whose labels include `tenant` are populated when tenant information is forwarded from Querier (see [Tenant Metrics](query.md#tenant-metrics)).
* This list focuses on Store Gateway–specific metrics. Generic process, gRPC, and object-store client metrics may also appear on the same `/metrics` endpoint.
* Label values such as `item_type` commonly include `postings`, `series`, and `expandedPostings`. `data_type` commonly includes `postings`, `series`, `chunks`, etc. `op` for postings compression is typically `encode` or `decode`.
