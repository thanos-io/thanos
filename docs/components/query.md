...
For example: If 2 prom replicas are scraping the same metrics (prom A and prom B) and if A is consistently reading metrics traffic every 15s at the 5 (every minute at 5, 20, 35, 50) and B is consistently reading the same metrics every 15s at the 14 (every minute at 14, 29, 44, 59) what is the impact to the total metric over a minute if the A process stops? Or if B stops?
