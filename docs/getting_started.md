...
If you want to build Thanos from source, make sure you have installed `bzr` and `git` (since `go` need `git` to download and `go mod` need `bzr` to do VCS thing, check with `which bzr` and `which git`),
